...
public record TradingProperties(
        ...
        // 고변동성 구간 ATR 배수
        @DecimalMin("0") @DefaultValue("4.0") BigDecimal atrMultHighVol,
        // 변동성 백분위 계산 lookback 길이
        @Positive @DefaultValue("30") int volRegimeLookback,
        // 고변동성 판정 백분위 임계값(0~1)
        @DecimalMin(value = "0", inclusive = false) @DecimalMax("1.0") @DefaultValue("0.7") BigDecimal volRegimeThreshold,
        // 레짐 밴드 비율
        @DecimalMin("0") @DecimalMax("0.999999") @DefaultValue("0.02") BigDecimal regimeBand,
