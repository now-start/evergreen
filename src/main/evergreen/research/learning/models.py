...
def window_features(candles: list[Candle]) -> tuple[Tensor, tuple[datetime, ...]]:
    if len(candles) < WARMUP:
        raise ValueError("모델 입력에는 연속된 확정 봉 55개 이상이 필요합니다")
...
class ChartModel(nn.Module):
    def __init__(self, architecture: str) -> None:
        ...
        if architecture not in ARCHITECTURES:
            raise ValueError("지원하는 모델은 mlp-v1 또는 cnn-v1입니다")
...
class ModelMetadata(BaseModel):
    ...
    architecture: Literal["mlp-v1", "cnn-v1"]
...
class TrainedModel:
    ...
    def predict(self, candles: list[Candle]) -> dict[datetime, Decimal]:
        features, times = window_features(candles)
        values = self.probabilities(features).tolist()
        return {time: Decimal(str(value)) for time, value in zip(times, values, strict=True)}


def validation_loss(model: ChartModel, features: Tensor, labels: Tensor) -> float:
    with torch.inference_mode():
        return float(nn.functional.binary_cross_entropy_with_logits(model(features), labels).item())
...
def train_model(
    ...
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        if validation_data is not None:
            model.eval()
            measured = validation_loss(model, *validation_data)
