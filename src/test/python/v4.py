"""Backtest V4 migrated from V2 architecture.

Strategy: daily regime transition + ATR trailing stop with weekly EMA trend filter.
"""
...
class BacktestServiceV4:
    def backtest_daily_strategy(self, daily_bars: list[CandleBar], params: StrategyParamsV4) -> BacktestResultV4:
        ...
        for i in range(n):
            ...
            filtered_buy = base_buy and weekly_bullish[i]
    ...
    def _build_weekly_filter(self, daily_bars: list[CandleBar], weekly_ema_len: int) -> tuple[list[float], list[float], list[bool]]:
        ...
        for i, bar in enumerate(daily_bars):
            ...
            day_to_week_idx[i] = len(week_keys) - 1

        weekly_ema_series = self._exponential_moving_average(weekly_closes, weekly_ema_len)
        weekly_bull_by_week = [math.isfinite(weekly_ema_series[i]) and weekly_closes[i] >= weekly_ema_series[i] for i in range(len(weekly_closes))]

        weekly_close_by_day = [math.nan] * n
        weekly_ema_by_day = [math.nan] * n
        weekly_bull_by_day = [False] * n
        for i in range(n):
            widx = day_to_week_idx[i]
            weekly_close_by_day[i] = weekly_closes[widx]
            weekly_ema_by_day[i] = weekly_ema_series[widx]
            weekly_bull_by_day[i] = weekly_bull_by_week[widx]
...
class UpbitDataServiceV4:
    def __init__(self) -> None:
        self._last_request_at_ms = 0

    def load_bars(self, config: BacktestConfigV4) -> list[CandleBar]:
...
@dataclass(frozen=True)
class BacktestRunOutputV4:
    validation_result: BacktestResultV4
    test_result: BacktestResultV4
    full_result: BacktestResultV4
    selected_params: StrategyParamsV4
    candidates: list[GridSearchRowV4]
...
class BacktestRunnerV4:
    def __init__(
        ...
        upbit_data_service: UpbitDataServiceV4 | None = None,
        backtest_service: BacktestServiceV4 | None = None,
        grid_search_service: GridSearchServiceV4 | None = None,
    ) -> None:
        self.config = config
        self.upbit_data_service = upbit_data_service or UpbitDataServiceV4()
        self.backtest_service = backtest_service or BacktestServiceV4()
        self.grid_search_service = grid_search_service or GridSearchServiceV4(self.backtest_service)

    def run(self) -> BacktestRunOutputV4 | None:
