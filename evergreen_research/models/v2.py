"""V2 model: EMA 레짐 전환과 ATR 추적 손절 전량 진입/청산."""
...
JAVA_PARAM_FIELDS = {"regimeEmaLen", "atrPeriod", "atrTrailMultiplier", "regimeBand"}
...
class StrategyModelV2:
    def evaluate(self, bars: list[CandleBar], params: StrategyParamsV2) -> list[ModelSignal]:
        ...
        for i, bar in enumerate(bars):
            ...
            previous_units = 0 if i == 0 else int(round((0 if i == 1 else target_units_close[i - 2])))
...
def iter_parameter_grid(config: BacktestConfigV2) -> Iterator[StrategyParamsV2]:
    ma_len_values = parse_positive_int_range(config.grid_ma_len_range, "grid_ma_len_range")
    atr_period_values = parse_positive_int_range(config.grid_atr_period_range, "grid_atr_period_range")
    atr_trail_values = parse_double_range(config.grid_atr_trail_mult_range)
    regime_band_values = parse_double_range(config.grid_regime_band_range)
    for ma_len in ma_len_values:
        for atr_period in atr_period_values:
            for atr_trail_multiplier in atr_trail_values:
                for regime_band in regime_band_values:
                    yield StrategyParamsV2(
