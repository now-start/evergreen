...
승인만으로 실거래 비활성화 설정이 활성화되지는 않는다.
이미 활성화된 감독 루프가 승인 누락으로 대기 중이면 다음 재검증에서 계좌 검증을 이어간다.
Config Server 값을 false로 바꾸는 것만으로 실행 중인 루프가 멈추지는 않으므로,
최초 승인 전에 앱을 중지하거나 false 설정으로 재시작해 실제 비활성화를 확인한다.
다음 워커 사이클에서 계좌·미체결 주문·호가를 검증한 뒤 상태를 자동 생성한다.
이 사이클은 주문하지 않으며, 이후 사이클부터 기존 실거래 설정과 전략에 따라 판단한다.
기존 상태가 있으면 잔고·미확정 주문을 대사하며 이어서 실행하고 상태를 덮어쓰지 않는다.
거래 루프 오류는 종류와 관계없이 주문 보류 → 대기 → 상태 재검증 → 정상 확인 후 자동 재개로 처리한다.
계좌 불일치·상태 손상도 재검증하며, 검증 실패가 지속되는 동안 주문하지 않는다.
DB 상태·원금·손실 한도를 임의로 초기화하지 않는다. 공통 기준은 [재시도·복구 정책](trading-recovery.md)을 따른다.
Config 변경도 실행 중 자동 반영하지 않으며 비활성화하려면 앱을 중지하거나 false 설정 후 재시작한다.

`/actuator/info`의 `trading`은 `disabled`, `starting`, `running`, `recovering`,
`halted`, `failed`, `stopping`, `stopped`와 마지막 완료 사이클 시각·결과, 고정 원인 코드·오류 타입,
연속 실패 횟수(`attempt`)와 대기 간격(`retry_seconds`)을 표시한다.
...
타임아웃은 10초, SDK 내부 자동 재시도는 0회이며 리다이렉트·환경변수 프록시를 사용하지 않는다.
429 등 일시 오류는 워커의 공통 복구 경계에서 대기 후 재검증한다.
복구 시 DB의 미확정 주문을 먼저 조회하며 주문 POST를 재전송하지 않는다.
...
2. 기본 돌파는 확정169봉의168시간 고점 진입 /48시간 저점 청산,
   early3는 기본 확정200봉을 읽고, DB의 마지막 처리 봉이 오래됐다면
   첫 미처리 봉의 이전168봉까지 추가 조회해 보호 상태·96시간 청산·48시간 리셋을 복원한다.
...
### 배포 순단·장시간 중단 후 복구 (2.2.1)

기존 DB 커서(`buffer.last_bar`)를 유지한 채 놓친 확정 시간봉을 시간순으로 처리한다.
조회 시작은 `min(현재 정시−200시간, 마지막 처리 봉 종료−168시간)`이다.
[업비트 분 캔들 API](https://docs.upbit.com/kr/reference/list-candles-minutes)의 최대200개와
배타적 `to` 경계에 맞춰 과거 방향으로 페이지를 읽고, 추가 호출 간0.12초 대기한다.
각 페이지와 전체 구간의 누락·중복·미확정·범위 오류를 검사하며, 결측을 보간하거나
커서를 현재로 강제 이동하지 않는다. 조회/검증 실패 시 기존 처리 커서는 유지된다.

early3는 정시 주문 창 밖에서도 상태와 차트 기록을 복구한다. 과거 매수 신호는
기록만 남기며, 최신 봉도 신호 유효시간이 지나면 매수하지 않는다. 이미 관측한
보유분의 청산 조건은 예약으로 유지하고, 실제 잔고·최신 호가·DB 잠금 검사를 거쳐 처리한다.
이 복구는 중단 중의 체결을 가정하지 않으며 그동안의 손실이나 청산 가격을 보장하지 않는다.

복구 차트는 최대32봉씩 기존 감사 테이블의 여러 행에 저장한다. 모든 행과 최종 상태는
하나의 트랜잭션으로 커밋하며 중간 실패 시 모두 롤백한다. 각 행의 `state`는 해당 봉의
중간 상태가 아니라 이번 복구의 최종 상태이고, 봉별 관측은 `detail.chart`에 있다.
커밋 성공 뒤에만 Loki 로그를 전송한다. 커밋 직후 프로세스가 종료되면 DB에는 기록이
있어도 일부 Loki 로그가 없을 수 있다(전송 보장 outbox는 아님).
재배포로 DB를 초기화하지 않으며 기존 미확정 주문 대사·중복 주문 방지·20% 중단 기준은 유지한다.
외부 통신·인증·데이터 오류는 [공통 재시도·복구 정책](trading-recovery.md)으로 처리한다.
