...
먼저 JSON을 백업하고 적용한다. 운영의 구형 Spring용 `이전 버전` 구역과 11개 패널은
사용자 요청으로 제거한다. 다른 서비스 대시보드와 원본 Loki/DB 이력은 삭제하지 않는다.
토큰·DB 자격 증명은 대시보드나 저장소에 넣지 않는다.

첫 화면은 마지막 워커 이벤트·판단 시각·판단 당시 보유·마지막 체결의 4개 요약 카드 →
손익 카드·집계 기준 → 가격·매매 차트 → 실제 체결 순서다. 카드는 선택 기간의 마지막 기록이며 현재 정상 상태를
보장하지 않는다. 기록 시각을 함께 표시하고, 수집되지 않은 값은 0이나 정상으로 채우지 않는다.
운영 로그·전략 판단 상세·조회 안내는 접힌 구역에 두고,
체결 표에서는 기술 식별자·관측 시각 등을 숨겨 핵심 정보만 표시한다.
쿼리·중복 제거·실제 시각 기준은 동일하며, 원본 기록은 삭제하지 않는다.
...
| `account_performance` | 잔고 대사와 최신 호가 검증을 통과한 정상 평가 사이클의 `valuation` 커밋 뒤 | 기준 원금, 추정 순자산, 누적 실현손익, 미실현손익, 총손익·총수익률, 누적 지급 수수료, 운용 시작·관측 시각, 집계 상태 |

### 손익의 기준

- **누적 실현손익:** 매도 대금 − 매도 수수료 − 매도 수량의 이동평균 매수 원가.
  매수 원가는 매수 수수료를 포함한다. 일부 체결 후 취소된 주문은 실제 체결분만 반영한다.
- **미실현손익(추정):** 보유 BTC × 최우선 매수호가 × (1 − 예상 매도 수수료율) − 잔여 매수 원가.
- **총손익(추정):** 현재 KRW + 위 방식의 BTC 평가금액 − 최초 운용 원금.
- **총수익률(추정):** (추정 순자산 / 최초 운용 원금 − 1) × 100. 표시 단위는 퍼센트이다.
- **누적 지급 수수료:** 확정 주문에 실제 지급한 수수료 합계. 예상 청산 수수료와 구별한다.

모두 단일 실행 DB/계좌의 최초 초기화 이후 누적값이며 **선택 기간 수익률이나 전략별 성과가 아니다.**
전략 변경 전 거래도 포함한다. 평가는 최우선 호가 기준이며 호가 깊이·슬리피지를 반영한 실제
청산 가능 금액을 보장하지 않는다. 거래 엔진과 동일한 잔고 대사 오차 허용치(KRW 0.01, BTC 1 satoshi)가 적용된다.

워커는 기존 DB의 `initialized`, `order-terminal` 이력을 순서대로 읽어 장부를 복원한다.
재시작마다 최초부터 복원하며 실행 중에는 이벤트 기본 키 커서 이후만 읽는다. 신규 테이블이나
실행 State 필드는 추가하지 않으므로 이전 이미지의 상태 파싱/롤백 형식도 유지된다.
최초 원금·매수 원가가 없거나, 주문 순번/잔고가 맞지 않거나, 외부 잔고 변동 기록이 있으면
손익은 `null`, 상태는 `unavailable`이다. 관련 이력이 한 번의 조회에서 10,000행을 넘으면
부분 합계 대신 `history_limit_exceeded`로 표시한다. DB 조회/잠금 오류는 기존처럼 전파한다.

기록은 INFO JSON으로 기존 OTel → Loki 경로를 사용한다. 손익 카드는 최신 로그 한 건의 누적값을
표시하므로 Loki의 과거 주문 로그 누락·배포 순단 때문에 합계가 초기화되지 않는다. 과거 시점의
계좌 평가를 소급 생성하지는 않는다. 새 버전 배포 후 다음 정상 평가부터 수집되며, 대기 주문이나
워커 중단 중에는 새 평가가 없으므로 반드시 마지막 관측 시각을 확인한다. 데이터 없음은 0이 아니다.
...
MDD 시계열, 모델 국면 확률, 알림 규칙은 이번 템플릿에 포함하지 않는다.
