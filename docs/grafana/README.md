...
`early3-loki-dashboard.json`은 **Grafana의 DB 연결 없이** 기존 Loki만 사용한다.
운영 대상은 기존 **에버그린 트레이딩** (`evergreen-trading-plot`)이다.
이 파일은 현재 Python 워커용 패널만 포함한다. 기존 운영 대시보드에 적용할 때는
먼저 JSON을 백업하고, 기존 패널은 접힌 이전 버전 구역에 보존한다.
토큰·DB 자격 증명은 대시보드나 저장소에 넣지 않는다.

첫 화면은 마지막 워커 이벤트·판단 시각·판단 당시 보유·마지막 체결의 4개 요약 카드 →
가격·매매 차트 → 실제 체결 순서다. 카드는 선택 기간의 마지막 기록이며 현재 정상 상태를
보장하지 않는다. 기록 시각을 함께 표시하고, 수집되지 않은 값은 0이나 정상으로 채우지 않는다.
운영 로그·전략 판단 상세·조회 안내는 접힌 구역에 두고,
체결 표에서는 기술 식별자·관측 시각 등을 숨겨 핵심 정보만 표시한다.
쿼리·중복 제거·실제 시각 기준은 동일하며, 원본 기록이나 기존 패널을 삭제하지 않는다.

- JSON 로그 본문을 추출하고 가격 문자열을 숫자로 변환한다. null은 0으로 바꾸지 않는다.
- 로그 검색은 Loki 기록 시각, 차트 표시와 추가 범위 필터는 실제 봉/체결 시각 기준이다.
  늦게 수집된 로그가 검색 범위 밖에 있으면 표시되지 않을 수 있다.
- `event_id`별 관측 시각이 가장 늦은 기록을 선택한다. 시간 버킷 평균으로 체결을 합치지 않는다.
- 가격선과 실제 매수/매도 점을 분리하며 체결 시각 없는 구형 주문은 제외한다.
- 쿼리당 최근 5,000행, 워커 로그 1,000행으로 제한한다. 긴 범위는 나눠 조회한다.
  제한에 도달하면 일부 기록이 빠질 수 있으므로 완전한 거래 장부로 사용하지 않는다.
- 자동 새로고침은 기본적으로 꺼져 있다. 새 차트 로그 배포 전에는 빈 차트가 정상이다.
  빈 결과 자체가 워커 정상이나 거래 없음을 뜻하지 않는다.
- Loki 누락·보존 만료는 이 대시보드에서 복원할 수 없다. 거래 프로그램의 DB 저장은 유지한다.

아래 MariaDB SQL 템플릿은 **선택 사항**이다. Loki 전용 구성에는 DB 데이터 소스나 계정이 필요 없다.

변환 계약 검증: 임시 디렉터리에 `@grafana/data@13.2.1`과 `rxjs`를 설치한 뒤
`node docs/grafana/verify-loki-dashboard.cjs /절대/임시경로/node_modules`로 실행한다.
JSON 추출 설정을 확인하고, 추출 직후 형태의 가짜 데이터에 실제 Grafana 변환을 적용해
시각·null·중복·매수/매도 분리·표·빈 입력을 검증한다. 운영 로그 수집이나 화면 렌더링 검증을 대신하지 않는다.
...
## 선택 사항: MariaDB 기반 템플릿 연결 순서

**이번 Loki 전용 대시보드 적용에는 아래 절차를 실행하지 않는다.**
DB 직접 조회를 별도로 선택한 경우에만 사용하는 대안이다.
