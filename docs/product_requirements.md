...
현재 프로젝트는 v1~v6 전략, Upbit 연동, 주문/체결/포지션 저장, 구조화 로그를 갖추고 있다. 다만 운영 안전성 관점에서는 API 계약 변경 대응, 주문 한도 정책, rate limit 대응, 문서화된 운영 절차가 제품 요구사항으로 명확해야 한다.
...
- 현재 지원 전략은 v1~v6이며, 전략별 파라미터 resolver를 통해 운영 파라미터를 구성한다.
